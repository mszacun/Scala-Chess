...
class FenParsingTest extends Test("FenParsingTest")
{
	def StartingPositionFenParseTest =
	{
		...
		assert(board.board(Cord.fromString("D1")) == Board.WHITE_KING)
		assert(board.board(Cord.fromString("E1")) == Board.WHITE_QUEEN)
		...
		assert(board.board(Cord.fromString("D8")) == Board.BLACK_KING)
		assert(board.board(Cord.fromString("E8")) == Board.BLACK_QUEEN)
		...
		// castle rights
		assert(board.castleRights.count((right : Boolean) => right) == 4)
